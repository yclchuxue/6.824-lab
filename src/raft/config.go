...
func (cfg *config) applier(i int, applyCh chan ApplyMsg) {
	for m := range applyCh {
		if m.CommandValid == false {
			...
			if err_msg != "" {
				log.Fatalf("A apply error: %v", err_msg)
...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		...
		if err_msg != "" {
			log.Fatalf("B apply error: %v", err_msg)
			cfg.applyErr[i] = err_msg
			// keep reading after error so that Raft doesn't block
			// holding locks...
		}
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		//DEBUG(dLog2, "S%d index(%d) committed count(%v) cmd1(%v) ok(%v)\n",i,index, count, cmd1, ok)
		cfg.mu.Unlock()
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			if retry == false {
				cfg.t.Fatalf("A one(%v) failed to reach agreement", cmd)
			}
		} else {
	...
	if cfg.checkFinished() == false {
		DEBUG(dLog, "AAAAAAAAAAAAAAAAAB\n")
		cfg.t.Fatalf("BA one(%v) failed to reach agreement", cmd)
		DEBUG(dLog, "AAAAAAAAAAAAAAAAA\n")
	}
	return -1
}
